...
- After persisting to disk, the route also notifies an n8n workflow: `notifyN8n()` POSTs `{ guest_name, group_size, booking_time }` (booking_time = `date`+`time` combined into one ISO 8601 string with an explicit `-03:00` offset) to `N8N_RESERVATION_WEBHOOK_URL`, with an `x-webhook-secret: N8N_WEBHOOK_SECRET` header — both read from `.env.local` (see `.env.example`), never hardcoded. This call is fire-and-forget/best-effort: a failure is only logged server-side and never affects the response already sent to the client, since the reservation is already durably saved by that point. The webhook is called from the server only — never from the browser.
